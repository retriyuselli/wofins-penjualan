...
class FinanceController extends Controller
{
    ...
    {
        $this->authorizeFinance('viewAny', DataPembayaran::class);
    ...
    {
        $this->authorizeFinance('viewAny', Order::class);
        /** @var User $user */
        $user = $request->user();
    ...
    {
        $this->authorizeFinance('viewAny', Prospect::class);
        $data = $request->validate([
            'status' => ['nullable', 'string', 'max:40'],
            'per_page' => ['nullable', 'integer', 'min:1', 'max:50'],
        ]);
    ...
    public function prospectShow(int $id): JsonResponse
    {
        ...
        \Illuminate\Support\Facades\Gate::authorize('view', $prospect);
    ...
    public function prospectUpdate(Request $request, int $id): JsonResponse
    {
        ...
        \Illuminate\Support\Facades\Gate::authorize('update', $prospect);
    ...
    public function paymentProof(Request $request, int $id): Response
    {
        ...
        \Illuminate\Support\Facades\Gate::authorize('view', $payment);
    ...
    public function vendorShow(int $id): JsonResponse
    {
        ...
        \Illuminate\Support\Facades\Gate::authorize('view', $vendor);
    ...
    {
        $this->authorizeFinance('viewAny', Piutang::class);
        $data = $request->validate([
    ...
    public function piutangShow(int $id): JsonResponse
    {
        ...
        \Illuminate\Support\Facades\Gate::authorize('view', $piutang);
    ...
    private function authorizeFinance(string $ability, string $modelClass): void
    {
        $user = request()->user();
        if ($user && method_exists($user, 'roles') && $user->roles()->whereIn('name', ['super_admin', 'Finance', 'admin_am', 'admin'])->exists()) {
            return;
        }

        \Illuminate\Support\Facades\Gate::authorize($ability, $modelClass);
    }
}
